...
//	package 접근 권한은 같은 package에서는 public처럼 사용되고 다른 package에서는 
//	private처럼 사용된다.
...
//		결론: 부모 클래스가 자식 클래스를 제어할 수는 있지만 자식 클래스가 부모 클래스를
//			  제어할 순 없다.
...
//		이때, casting 시켜서 대입하면 대입은 가능해지고 정상적인 downcasting이 아닌 경우 문법적으로
//		오류는 발생하지 않지만 ClassCastException이 발생한다.
...
//		instanceof 안 쓰고 객체가 형변환이 가능한지 try & catch로 알아볼 수 있다.		
		try {
			Sub sub2 = (Sub) base;
			sub2.say();
...
//		downcsting은 부모 클래스 타입으로 upcasting된 것을 다시 자식 클래스 타입에 넣어주는 것을 말한다.
