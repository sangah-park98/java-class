...
//	static이 붙으면 new해서 객체 만들 필요가 없음
	
//	리턴 타입은 메소드가 실행되고 난 후 결과의 자료형을 말한다.
...
//	main 메소드가 메모리에 적재되어 있지 않으면 프로그램의 시작점은 main() 메소드를 호출할 수 없으므로
...
//		1일이 출력될 위치(요일)을 맞추기 위해 1일의 요일만큼 반복하며 빈칸(날짜당 4칸)을 출력한다.
