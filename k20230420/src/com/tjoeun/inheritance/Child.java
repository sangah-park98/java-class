...
//	super()와 this()는 반드시 생성자의 첫 문장으로 써야 한다 => 가장 먼저 실행된다.
//	부모 클래스의 생성자가 자식 클래스의 생성자보다 먼저 실행된다.
//	부모 클래스에서 기본 생성자를 정의하지 않으면 자식 클래스에서 super()가 실행될 때 에러가 발생된다.
...
//	1) 부모 클래스에서 상속받은 필드의 접근 권한이 private 권한일 경우 자식 클래스에서 직접
...
//	2) 부모 클래스에서 private 권한으로 설정된 모든 필드에 setter 메소드가 정의되어 있다면
//	   setter는 메소드이므로 public 권한으로 작성되었기 때문에 자식 클래스에서 실행할 수
//	   있으므로 부모 클래스에서 상속받은 private 권한의 필드를 setter를 이용해서 초기화시킬 수 있다.
	/*
	public Child(String name, boolean gender, int age, String nickname) {
		setGender(gender);
		setName(name);
