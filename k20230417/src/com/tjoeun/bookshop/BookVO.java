...
//	VO(Value Object) 클래스: 1건의 데이터와 데이터를 처리하는 메소드를 모아놓은 클래스이다.= 라면 1봉지
...
//	초기화를 하지 않아도 숫자는 0, 문자는 공백, boolean은 false, 클래스로 만든 객체(참조변수)는 null로 
//	자동 초기화된다.
//	라면 봉지 안에 있는 소중한 사리면(설계도)
...
//	생성자는 리턴타입을 사용하지 않고 생성자 내부에서도 return을 사용하지 않는다.
...
//	@Override 어노테이션은 이 메소드가 부모 클래스에 상속받은 메소드를 @Override한 메소드임을 의미한다.
...
//		소수점 아래 숫자를 출력하려면 "."을 입력하고 보고 싶은 자리만큼 "0"을 입력한다.
