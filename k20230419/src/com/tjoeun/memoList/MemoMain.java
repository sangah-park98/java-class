...
// 	수정과 삭제는 코드가 비슷하기 때문에 삭제 먼저 쓰고 나중에 수정에 붙여서 추가하는 게 편함
public class MemoMain {
	
//	 글 목록을 저장하는 memoList 클래스 객체는 MemoMain 클래스의 여러 메소드에서 사용해야 하기 
//	 때문에 MemoMain 클래스의 필드로 선언해야 한다.
...
//			삭제하기 위해 입력한 비밀번호(password)와 삭제할 글의 비밀번호(original)를 비교한다.
			if(password.equals(original.getPassword())) {
