...
interface Repairable { // SVC가 수리 가능한 유닛 클래스에 구현한다.
//	아무것도 코딩하지 않는다.
}
...
//	상수를 초기화시키는 방법은 3가지가 있는데 
//	1번째 방법은 상수 선언시 "="을 사용해서 초기화시키는 방법이고
//	2번째 방법은 객체가 생성될 때 생성자에서 초기화시킨다.
...
//	부모 클래스에 기본 생성자가 없기 때문에 상속 받은 자식 클래스에서 자동으로 들어있는 super();
//	을 실행하다가 에러가 발생해 빨간 줄이 나온다. => 클릭하여 해결(int hp)가 들어있는 생성자로 변경해줌
	public Unit(int hp) {
//		④ Unit 클래스의 생성자에서 인수로 넘어온 150을 받아서 MAX_HP를 초기화시킨다.
		MAX_HP = hp; // Unit별로 MAX_HP를 다르게 설정하기 위해 초기화를 바로 선언하지 않았다.
	}
...
// 기본 생성자가 각각의 클래스마다 다르게 들어갈 때는 기본 생성자가 부모에 없기 때문에 에러 16:40
...
//		생성자 GroundUnit(int hp)를 호출한다.
		super(150);
//	⑦ Tank의 MAX_HP가 150으로 초기화되었으므로 현재 HP(상속받은 currentHP)를 MAX_HP로 
//	초기화시킨다.
		currentHP = MAX_HP;
...
//	GroundUnit 클래스를 상속받아 SCV를 만든다. (SCV는 수리가 가능)
