...
//		선언한 배열의 초기치를 지정해서 전달해야 하는 번거로움이 있다.
...
//	위 4개의 메소드는 인수를 모두 변수로 받고 있기 때문에 많은 수의 오버로딩이 필요했다.
//	하지만 인수를 배열로 받으면 많은 수의 오버로딩이 필요없다.
...
//	가변 인수는 인수 목록의 맨 뒤에 딱 1번만 사용할 수 있다.
	private static int total(int ... data) { //(int ... data, int a)는 안되고 (int a, int ... data)는 가능
		int total = 0;
		for(int i=0; i<data.length; i++) {
			total += data[i];
		}
