...
//		Plastic을 재료로 사용하는 3D 프린터 객체를 만든다.
...
//		위에서 재료가 생성되어 있으므로 다시 만들지 않는다.
//		Plastic plastic = new Plastic();
//		Powder powder = new Powder();
		
		threeDPrinter.setMaterial(powder); //10:11 upcasting
		System.out.println(printerPowder.getMaterial());
//		자식 객체를 부모 객체에 넣을 때(upcasting)는 별다른 문제가 발생되지 않지만
//		부모 객체에 저장된 자식 객체를 자식 객체에 넣을 때(downcasting)는 반드시 형변환 시켜서 넣어야 한다.
